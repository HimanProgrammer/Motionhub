...
Wait for my code, then begin.`,
  },
  {
    ...
    prompt:
`# CHECKMATE — Chess App Dashboard (Master Prompt for Claude, Next.js)
...
- File tree, then each file in full (app/page.js or a dashboard route, its CSS module, and any small subcomponents), then setup steps. Make sensible choices; don't ask questions.`,
  },
  {
    ...
    prompt:
`# PROPERTIX — Property Management Website (Master Prompt)

Build a complete, pixel-perfect, fully responsive marketing website for PROPERTIX, a full-service property management company. Premium, trustworthy, "we handle everything" energy — corporate but warm, never sterile.

## Tech Stack (IMPORTANT: plain CSS only — NO Tailwind, NO CSS framework)
- HTML5 semantic layout (or Next.js App Router in JavaScript — NO TypeScript — with CSS Modules).
...
- Icons: Remix Icon CDN (https://cdn.jsdelivr.net/npm/remixicon@4.2.0/fonts/remixicon.css).
- Vanilla JavaScript (IntersectionObserver reveals, mobile drawer, form validation).

## Palette
- Deep navy background: #0a1628, with a slightly lighter panel navy #0d1c31.
- Warm gold accent: #e8b04a (buttons, highlights, the "Properties," word, section underlines).
- Body text #e8ecf3, muted #97a3b6.
- Cards sit on the navy with a 1px rgba(255,255,255,.08) border and generous radius (14–16px).

## Assets (real, working URLs)
- Hero building: https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?w=1600&q=80
- Residential card: https://images.unsplash.com/photo-1512917774080-9991f1c4c750?w=1200&q=80
- Commercial card: https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?w=1200&q=80
- Maintenance card: https://images.unsplash.com/photo-1560518883-ce09059eeffa?w=1200&q=80
- About section: https://images.unsplash.com/photo-1580587771525-78b9dba3b914?w=1200&q=80
- Featured listings: https://images.unsplash.com/photo-1564013799919-ab600027ffc6?w=1200&q=80 , https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?w=1200&q=80 , https://images.unsplash.com/photo-1497366754035-f200968a6e72?w=1200&q=80

## Sections, in order
1. **Sticky nav** — house-outline SVG logo in a gold rounded square + "Propertix" wordmark with small-caps "MANAGEMENT" beneath. Links: Home (gold/active), About Us, Services, Properties, Owners, Tenants, Blog, Contact. Gold pill button "Get a Quote". Mobile: hamburger → full-screen drawer with blurred backdrop and staggered link fade-in.
2. **Hero** — split layout. Left: headline on three lines, "We Manage" / "Properties," (in gold) / "You Enjoy Peace."; subhead "Professional property management services that protect your investment and maximize your returns."; two buttons — gold "Our Services" + outlined "View Properties". Below them, two trust badges with icons: "Trusted Property Management" (shield) and "Happy Owners & Tenants" (people). Right: the hero building photo, full-bleed to the right edge, with a soft navy gradient blending it into the background. Staggered fade-up entrance for each element.
3. **Our Management Services** — centered heading with a short gold underline rule and a two-line subhead. Then a 4-column grid: three image-topped service cards (Residential Management, Commercial Management, Maintenance Services), each with a floating rounded icon tile overlapping the image's bottom-left corner, a title, two lines of copy, and a gold "Learn More →" link whose arrow slides on hover. The fourth cell is a solid gold panel: house icon, "Why Choose Us?", and a checklist (Experienced Management Team, Transparent Communication, Maximized Rental Income, Minimal Vacancy Rates, Reliable Maintenance) with white check marks, plus a small decorative plant + clock photo strip at its base.
4. **About** — split card on a panel background. Left: gold eyebrow "About Propertix", heading "We Take Care / So You Don't Have To" with a short gold underline, a paragraph about being full-service and protecting the investment, and a gold "More About Us" button. Right: the about photo, rounded, filling the card's right half.
5. **Featured Properties** — centered heading + gold rule + subhead. A 3-column grid of listing cards: photo, title (Modern Family Home / Downtown Office Space / Luxury Apartment), and a muted meta line with bullet separators ("3 Bed • 2 Bath • 2200 Sqft", "2000 Sqft • Prime Location", "2 Bed • 2 Bath • City View"). Cards lift and the photo scales gently on hover.
6. **Footer** — darker navy. Four columns: brand block (logo + one-paragraph blurb + circular social icons for Facebook, X, LinkedIn, Instagram), "Quick Links", "Our Services", "Useful Links", and a "Newsletter" column with a short line of copy, an email input, and a full-width gold "Subscribe" button. Bottom bar: centered "© 2025 Propertix Management. All Rights Reserved."

## Interactions & polish
- Scroll-reveal (IntersectionObserver) with a small stagger across cards in each grid.
- Buttons: gold fill lifts 2px with a soft gold glow on hover; outlined buttons fill faintly.
- Newsletter form validates the email inline and shows a success state — no page reload.
- Fully responsive: 4-col → 2-col → 1-col; hero stacks with the image below the copy on mobile.
- 60fps, transforms and opacity only. Accessible focus states, semantic landmarks, alt text on every image.

Output a single self-contained index.html (inline <style> + <script>) unless I asked for the Next.js version.`,
  },
];
